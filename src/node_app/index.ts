...
const openIdConnectUrl = "https://jde.circularedge.com:5001/auth/realms/AtomIQ";
...
export class App {
  ...
  makeAccessTokenRequest(
    ...
    return this.tokenHandler
      ...
      .then((response) => {
        log(
          `Access Token is ${response.accessToken}, Id Token is ${response.idToken}`
        );
        return response;
      });
  }

  makeRevokeTokenRequest(
    configuration: AuthorizationServiceConfiguration,
    refreshToken: string
  ) {
